...
module.exports = class PetController {
  static async create(req, res) {
    ...
    try {
      ...
      return res
        .status(201)
        .json({ message: "Pet cadastrado com sucesso!", data: newPet });
    } catch (error) {
  ...
  static async getAllUserAdoptions(req, res) {
    const pets = await Pet.find({ "adopter._id": req.user.id }).sort(
      "-createdAt",
    );
    return res.status(200).json(pets);
  }
  ...
  static async removePetById(req, res) {
    ...
    if (pet.user._id.toString() !== req.user.id) {
      return res
        .status(403)
        .json({ message: "Ação não permitida para este usuário." });
    }
  ...
  static async updatePet(req, res) {
    ...
    return res
      .status(200)
      .json({ message: "Pet atualizado com sucesso!", data: updatedPet });
  }
  static async schedule(req, res) {
    ...
    return res
      .status(200)
      .json({ message: "Visita agendada com sucesso!", data: pet });
  }
  ...
  static async concludeAdoption(req, res) {
    ...
    if (pet.user._id.toString() !== req.user.id) {
      return res
        .status(403)
        .json({ message: "Apenas o dono pode concluir a adoção." });
    }
    ...
    return res
      .status(200)
      .json({ message: "Adoção concluída com sucesso!", data: pet });
  }
};
